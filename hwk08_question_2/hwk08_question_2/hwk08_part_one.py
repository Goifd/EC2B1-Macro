...
!!!!!!!!! WHAT YOU SHOULD DO: LOOK FOR XXX AND REPLACE IT WITH CODE !!!!!!

In this assignment, you are asked to do the same calculations for different parameter values. 
Of course, you could just repeat the code. But a more efficient way to do this is to first define a function that does the operation.

The following is a very simple example of how to define a function in Python. Try creating some simple functions yourself before
...
x_try = 4
outcome1,outcome2 = function_simple(x_try)  
print(outcome1)
print(outcome2)
...
each of length dt. In code, this will involve a `for` loop. A simple example of a for loop is the following.
...
comments:
    - Note that 0 is the default starting point and "for t in range(5) does the same"
    - It stops when it reaches 5, that is, it will not execute for t=5
...
def iterate_forward(n, lam, phi, L0, A0, T, dt):
    ...
    for t in range(T-1):
        ...
        L_path[t+1] = L_path[t] + (n*L_path[t])*dt
...
dt = 0.01   # time step
T = 300 # set time some total time horizon
